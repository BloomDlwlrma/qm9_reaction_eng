...
if [ "$OVERWRITE_CSV" -eq 1 ] || [ ! -f "$PAIR_CSV" ]; then
    python3 "$SCRIPTS_DIR/_extract_pair_corr_eng.py" \
        "$ORCA_OUT_DIR"/*.out \
        --output "$PAIR_CSV" \
        --mode write
else
...
if [ "$OVERWRITE_CSV" -eq 1 ] || [ ! -f "$TRIPLES_CSV" ]; then
    python3 "$SCRIPTS_DIR/_extract_triples_correction.py" \
        "$ORCA_OUT_DIR"/*.out \
        --pair-sum \
        --output "$TRIPLES_CSV" \
        --mode write
...
# Note: The script _create_pair_hdf5.py now enforces saving to HDF5_DIR internally
python "$SCRIPTS_DIR/_create_pair_hdf5.py" \
    --pair-csv "$PAIR_CSV" \
    --pair-h5 "$PAIR_H5" \
...
echo "====================================================================="
echo "Workflow Completed Successfully, HDF5 outputs saved to: $HDF5_DIR  "
echo "====================================================================="
